...
use crate::object::{generate_object_id, Address, Object, ObjectId};
...
impl RelationGraphData {
    ...
    pub fn remove_relation(&mut self, target_sbt: &ObjectId, relation_type: &str) -> bool {
        ...
        self.relations
            .retain(|r| !(r.target_sbt == *target_sbt && r.relation_type == relation_type));
...
impl RelationGraph {
    ...
    pub fn new(owner_sbt: ObjectId, owner_address: Address, graph_type: String) -> Self {
        let id = generate_object_id(format!("graph:{}:{}", owner_sbt, graph_type).as_bytes());
...
impl UserRelationNetwork {
    ...
    pub fn add_relation(&mut self, target: Address, relation_type: &str, weight: u32) {
        ...
        self.relation_graph
            .add_relation(target_id, relation_type.to_string(), weight);
        self.touch();
    }
    ...
    pub fn remove_relation(&mut self, target: Address, relation_type: &str) -> bool {
        ...
        let removed = self
            .relation_graph
            .remove_relation(&target_id, relation_type);
        if removed {
            self.touch();
        }
        removed

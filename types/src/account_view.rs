...
impl AccountView {
    ...
    pub fn new(
        ...
        let valid_credential_count = credentials.iter().filter(|c| c.data.is_valid()).count();
        let graph_count = graphs.len();
        let total_relations = graphs.iter().map(|g| g.data.relation_count()).sum();
    ...
    pub fn bio(&self) -> Option<&String> {
        self.profile.as_ref().and_then(|p| p.data.bio.as_ref())
    }
    ...
    pub fn get_graph_by_type(&self, graph_type: &str) -> Option<&RelationGraph> {
        self.graphs.iter().find(|g| g.data.graph_type == graph_type)
    }
...
mod tests {
    use super::*;
    use crate::{
        coin::create_coin, object::generate_object_id, profile::create_kyc_credential,
        profile::create_profile, relation::create_social_graph,
    };
    ...
    fn test_account_view_creation() {
        ...
        let graphs = vec![create_social_graph(owner_id, alice.clone())];

        let view = AccountView::new(alice.clone(), Some(profile), vec![], coins, graphs);
    ...
    fn test_credentials() {
        ...
        let view = AccountView::new(alice.clone(), None, vec![kyc], vec![], vec![]);
    ...
    fn test_has_balance() {
        ...
        let coins = vec![create_coin(alice.clone(), 1000)];
    ...
    fn test_get_graphs() {
        ...
        social_graph
            .data
            .add_relation(bob_id, "follows".to_string(), 100);

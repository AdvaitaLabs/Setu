...
pub async fn handle(action: crate::TransferAction, _config: &Config) -> Result<()> {
    match action {
        crate::TransferAction::Submit {
            ...
            println!(
                "{}",
                "║              Submitting Transfer to Validator              ║".cyan()
            );
            println!(
                "{}",
                "╚════════════════════════════════════════════════════════════╝".cyan()
            ...
            let request = SubmitTransferRequest {
                ...
                resources: vec![format!("account:{}", from), format!("account:{}", to)],
            };
            ...
            match response {
                    ...
                    if resp.status().is_success() {
                        ...
                        if result.success {
                            ...
                            println!(
                                "{}  Message:     {:<45} {}",
                                "║".green(),
                                &result.message[..45.min(result.message.len())],
                                "║".green()
                            );
                            ...
                            println!(
                                "{}  Error: {:<48} {}",
                                "║".red(),
                                &result.message[..48.min(result.message.len())],
                                "║".red()
                            );
                            println!(
                                "{}",
                ...
                Err(e) => {
                    ...
                    println!(
                        "{}  Could not connect to Validator at {}                 {}",
                        "║".red(),
                        router,
                        "║".red()
                    );
                    println!(
                        "{}  Error: {:<48} {}",
                        "║".red(),
                        &e.to_string()[..48.min(e.to_string().len())],
                        "║".red()
                    );
                    println!(
                        "{}",

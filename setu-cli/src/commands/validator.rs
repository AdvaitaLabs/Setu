...
use crate::commands::keygen::{
    display_keypair, export_keypair, generate_keypair, load_keypair, recover_from_mnemonic,
    save_keypair,
};
use crate::config::Config;
use anyhow::Result;
use colored::Colorize;
use setu_rpc::{HttpRegistrationClient, RegisterValidatorRequest};

pub async fn handle(action: crate::ValidatorAction, _config: &Config) -> Result<()> {
    match action {
        crate::ValidatorAction::Keygen {
            output,
            id,
            stake,
            commission,
        ...
        crate::ValidatorAction::Register {
            key_file,
            network_address,
            network_port,
            router,
            ...
            println!(
                "  Network Address:  {}:{}",
                network_address.cyan(),
                network_port.to_string().cyan()
            );
            ...
            let stake_amount = keypair
                .metadata
                .get("stake_amount")
                .and_then(|v| v.as_u64())
                .unwrap_or(10000);
            let commission_rate = keypair
                .metadata
                .get("commission_rate")
                .and_then(|v| v.as_u64())
                .unwrap_or(10) as u8;
            ...
            let message = format!(
                "Register Validator: {}:{}:{}",
                keypair.node_id, keypair.account_address, timestamp
            );
            ...
            match client.register_validator(request).await {
                ...
                Err(e) => {
                    ...
                    println!(
                        "{} Make sure the validator service is running at {}",
                        "→".dimmed(),
                        router.dimmed()
                    );
                }
        ...
        crate::ValidatorAction::Status { id } => {
            ...
            println!(
                "{} Status query requires validator address",
                "!".yellow().bold()
            );

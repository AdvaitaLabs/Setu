...
use setu_rpc::{HttpRegistrationClient, RegisterSolverRequest};
...
pub async fn handle(action: crate::SolverAction, _config: &Config) -> Result<()> {
    match action {
        ...
        crate::SolverAction::Status { id } => {
            ...
            println!("{} Status query not yet implemented", "!".yellow().bold());
        ...
        crate::SolverAction::Heartbeat { id, load, router } => {
            ...
            println!("{} Heartbeat not yet implemented", "!".yellow().bold());

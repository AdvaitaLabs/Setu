...
impl MockEnclave {
    ...
    fn build_state_from_read_set(
        ...
        for entry in read_set {
            ...
            if let Some(hex_id) = hex_id {
                ...
                let coin_state: CoinState = bcs::from_bytes(&entry.value).map_err(|e| {
    ...
    async fn simulate_execution(
        ...
        for event in &input.events {
            ...
            let result = self
                .execute_single_event(event, &input.resolved_inputs, &mut diff)
                .await;
    ...
    async fn execute_single_event(
        ...
        if let Some(transfer) = &event.transfer {
            return self
                .execute_transfer_via_runtime(event, transfer, resolved_inputs, diff)
                .await;
        }
...
impl EnclaveRuntime for MockEnclave {
    async fn execute_stf(&self, input: StfInput) -> StfResult<StfOutput> {
        ...
        let gas_used = execution_time.as_micros() as u64 / 10
            + writes_count * 100
            + input.read_set.len() as u64 * 10;

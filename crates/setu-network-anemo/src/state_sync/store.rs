...
use super::{SerializedConsensusFrame, SerializedEvent, SerializedVote, StateSyncStore};
    ...
    async fn get_events_by_ids(
        ...
        let result = event_ids
            ...
            .filter_map(|id| {
                events_by_id
                    .get(id)
                    .and_then(|seq| events_by_seq.get(seq).cloned())
            })
            .collect();
...
mod tests {
    ...
    async fn test_event_by_id() {
        ...
        let events = store
            .get_events_by_ids(&["event_1".to_string(), "event_3".to_string()])
            .await
            .unwrap();

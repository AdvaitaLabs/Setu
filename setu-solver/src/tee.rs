...
use setu_enclave::{
    Attestation, EnclaveInfo, EnclaveRuntime, GasUsage, MockEnclave, SolverTask, StfInput,
    StfOutput,
};
...
impl TeeExecutor {
    ...
    pub async fn execute_solver_task(
        &self,
        task: SolverTask,
...
impl TeeExecutionResult {
    ...
    pub fn from_stf_output(output: StfOutput) -> Self {
        ...
        let state_changes: Vec<StateChange> = output
            .state_diff
            .writes
            .into_iter()
            .map(|w| StateChange {
            ...
            .chain(output.state_diff.deletes.into_iter().map(|k| StateChange {
                key: k,
                old_value: None,
                new_value: None,
            }))
    ...
    pub fn to_execution_result(&self) -> ExecutionResult {
        ExecutionResult {
            ...
            message: if self.events_failed == 0 {
                Some(format!(
                    "Processed {} events in {}μs",
                    self.events_processed, self.execution_time_us
                ))
            } else {
                Some(format!(
...
mod tests {
    use super::*;
    use setu_enclave::{EnclavePlatform, GasBudget, ResolvedInputs};

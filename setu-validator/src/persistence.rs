...
pub enum PersistenceError {
    ...
    AnchorPersistenceFailed { anchor_id: String, reason: String },
}
...
pub trait FinalizationPersister: Send + Sync {
    ...
    async fn persist_finalized_anchor(&self, anchor: &Anchor) -> PersistenceResult<()> {
        ...
        let events_with_depths: Vec<_> = anchor
            .event_ids
            .iter()
            .filter_map(|id| match dag.get_event(id) {
                Some(event) => {
                    let depth = dag
                        .get_depth(id)

...
use setu_rpc::{
    CoinBalance, CreditChange, GetAccountRequest, GetAccountResponse, GetBalanceRequest,
    GetBalanceResponse, GetCredentialsRequest, GetCredentialsResponse, GetCreditRequest,
    GetCreditResponse, GetPowerRequest, GetPowerResponse, PowerChange, ProfileInfo,
    RegisterUserRequest, RegisterUserResponse, SubmitTransferRequest, TransferRequest,
    TransferResponse, UserRpcHandler,
};
...
impl UserRpcHandler for ValidatorUserHandler {
    async fn register_user(&self, request: RegisterUserRequest) -> RegisterUserResponse {
        ...
        self.network_service
            .apply_event_side_effects(&event_id)
            .await;
        ...
        info!(
            "║  Address:    {:^44} ║",
            &request.address[..20.min(request.address.len())]
        );
        info!(
            "║  Event ID:   {:^44} ║",
            &event_id[..20.min(event_id.len())]
    ...
    async fn get_power(&self, request: GetPowerRequest) -> GetPowerResponse {
        ...
        GetPowerResponse {
            ...
            power: 100,
            rank: Some(42),
            recent_changes: vec![PowerChange {
                amount: 10,
    ...
    async fn get_credit(&self, request: GetCreditRequest) -> GetCreditResponse {
        ...
        GetCreditResponse {
            ...
            recent_changes: vec![CreditChange {
                amount: 5,

...
use crate::protocol::{NetworkEvent, SerializedEvent, SetuMessage};
...
pub trait MessageHandlerStore: Send + Sync + 'static {
    ...
    async fn get_events_by_ids(&self, event_ids: &[String])
        -> Result<Vec<SerializedEvent>, String>;
    ...
    S: MessageHandlerStore,
{
    ...
    pub fn new(store: Arc<S>, local_node_id: String, event_tx: mpsc::Sender<NetworkEvent>) -> Self {
        Self {
            store,
            local_node_id,
            event_tx,
    ...
    async fn handle_message(
        &self,
        message: SetuMessage,
    ) -> Result<Option<SetuMessage>, HandlerError> {
        match message {
            ...
            SetuMessage::CFVote { vote } => {
                ...
                if let Err(e) = self
                    .event_tx
                    .send(NetworkEvent::VoteReceived {

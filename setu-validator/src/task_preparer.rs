...
use setu_types::task::{
    GasBudget, MerkleProof, ReadSetEntry, ResolvedInputs, ResolvedObject, SolverTask,
};
use setu_types::{Event, EventType, ObjectId, SubnetId};
use std::sync::Arc;
use tracing::{debug, info, warn};

// Re-export StateProvider and CoinInfo from storage
pub use setu_storage::{CoinInfo, SimpleMerkleProof, StateProvider};
...
impl TaskPreparer {
    ...
    pub fn new_for_testing(validator_id: String) -> Self {
        use setu_storage::{init_coin, GlobalStateManager, MerkleStateProvider};
        use std::sync::RwLock;
    ...
    pub fn prepare_transfer_task_with_coin_type(
        ...
        let sender_coins = match coin_type {
            Some(ct) => self
                .state_provider
                .get_coins_for_address_by_type(&transfer.from, ct),
        ...
        let coin_data = self
            .state_provider
            .get_object(&selected_coin.object_id)
            .ok_or(TaskPrepareError::ObjectNotFound(hex::encode(
                &selected_coin.object_id,
            )))?;

        let merkle_proof = self
            .state_provider
            .get_merkle_proof(&selected_coin.object_id);
...
mod tests {
    ...
    fn test_insufficient_balance() {
        ...
        match result {
            Err(TaskPrepareError::InsufficientBalance {
                required,
                available,
            }) => {

...
use setu_storage::{
    AnchorStore, AnchorStoreBackend, CFStoreBackend, EventStore, EventStoreBackend,
    GlobalStateManager, MerkleStore, RocksDBAnchorStore, RocksDBCFStore, RocksDBEventStore,
    RocksDBMerkleStore, SetuDB,
};
use setu_types::{
    event::VLCSnapshot, Anchor, ConsensusConfig, Event, EventPayload, EventStatus, EventType,
    NodeInfo, SubnetId,
};
...
async fn test_recovery_single_anchor() {
    ...
    // Phase 2: Simulate restart - create new validator with same DB path
    {
        ...
        assert!(
            anchor.is_some(),
            "Latest anchor should exist after recovery"
        );
...
async fn test_recovery_multiple_anchors() {
    ...
    // Phase 1: Create multiple anchors
    {
        ...
        for anchor_idx in 1..=3i32 {
            ...
            let prev_anchor = if anchor_idx == 1 {
                None
            } else {
                Some(format!("anchor-{:03}", anchor_idx - 1))
            };
            let anchor = create_test_anchor(
                &format!("anchor-{:03}", anchor_idx),
                (1..=3)
                    .map(|i| format!("evt-{:03}", base_evt_id + i))
                    .collect(),
                anchor_idx as u64,
                prev_anchor,
...
async fn test_event_store_persistence() {
    ...
    let event_ids: Vec<String>;
    {
        ...
        for (idx, event_id) in event_ids.iter().enumerate() {
            ...
            event_store
                .store_with_depth(event, (idx + 1) as u64)
                .await
                .unwrap();
        }
    ...
    // Phase 2: Reopen and verify
    {
        ...
        for event_id in &event_ids {
            ...
            assert!(
                event.is_some(),
                "Event {} should exist after restart",
                event_id
            );
        }
...
async fn test_backend_api_compatibility() {
    ...
    let db = Arc::new(SetuDB::open_default(temp_dir.path()).expect("Failed to open RocksDB"));
    let rocks_event_store: Arc<dyn EventStoreBackend> =
        Arc::new(RocksDBEventStore::from_shared(db.clone()));
    let rocks_anchor_store: Arc<dyn AnchorStoreBackend> =
        Arc::new(RocksDBAnchorStore::from_shared(db.clone()));

    // Both should support the same operations
    let test_event = create_test_event("compat-test", "validator-1", vec![]);
    let test_anchor = create_test_anchor(
        "compat-anchor",
        vec!["compat-test".to_string()],
        1,
        None,
        50,
    );
    ...
    // Store in both
    memory_event_store
        .store_with_depth(test_event.clone(), 1)
        .await
        .unwrap();
    rocks_event_store
        .store_with_depth(test_event.clone(), 1)
...
async fn test_vlc_state_restoration() {
    ...
    // Phase 2: Recovery and verify VLC state
    {
        ...
        assert!(
            current_time >= 12345,
            "VLC should be restored to at least 12345, got {}",
            current_time
        );

        info!(
            "Phase 2: VLC restored, current logical_time={}",
            current_time
        );
    }
...
async fn test_recovery_idempotency() {
    ...
    // Setup: Store some data
    {
        ...
        let anchor =
            create_test_anchor("idempotent-anchor", vec!["evt-1".to_string()], 1, None, 100);
        anchor_store.store(anchor).await.unwrap();
    }

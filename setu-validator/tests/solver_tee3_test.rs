...
use setu_solver::{GasBudget, ResolvedInputs, SolverTask, TeeExecutionResult, TeeExecutor};
use setu_types::{Event, EventType, VLCSnapshot};
use setu_types::{ObjectId, SubnetId};
use setu_types::{Transfer, TransferType};
use setu_validator::task_preparer::{CoinInfo, SimpleMerkleProof, StateProvider, TaskPreparer};
...
fn create_test_transfer(id: &str, from: &str, to: &str, amount: u64) -> Transfer {
    Transfer::new(id, from, to, amount).with_type(TransferType::FluxTransfer)
}
...
async fn test_dependency_derivation_with_history() {
    ...
    use setu_storage::{state_provider::init_coin, GlobalStateManager};
    use std::sync::RwLock;
    ...
    assert_eq!(task.event.parent_ids.len(), 1, "Should have one dependency");
    assert_eq!(
        task.event.parent_ids[0], "event-previous-123",

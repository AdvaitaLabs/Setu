...
use crate::rocks::core::{ColumnFamily, IntoSetuResult, SetuDB, StorageOperation};
...
impl RocksObjectStore {
        ...
        if ids.len() < original_len {
            if ids.is_empty() {
                self.db
                    .delete(cf, key)
                    .map_err(|e| {
                        e.with_operation(StorageOperation::Delete)
                            .with_cf(cf.name())
                    })
                    .into_setu_result()?;
            } else {
    ...
    fn add_subnet_to_user_index(&self, user: &Address, subnet_id: &SubnetId) -> SetuResult<()> {
        ...
        let mut subnet_ids: Vec<SubnetId> = self
            .db
            .get(cf, user)
            .map_err(|e| e.with_operation(StorageOperation::Get).with_cf(cf.name()))
            .into_setu_result()?
    ...
    fn remove_subnet_from_user_index(
        &self,
        user: &Address,
        subnet_id: &SubnetId,
    ) -> SetuResult<()> {
    ...
    fn add_to_owner_type_index(
        ...
        let mut ids: Vec<ObjectId> = self
            .db
            .get_raw(ColumnFamily::CoinsByOwnerAndType, &key)
            .map_err(|e| {
                error!(
                    owner = %owner,
                    coin_type = %coin_type,
        ...
        if !ids.contains(object_id) {
            ids.push(*object_id);
            self.db
                .put_raw(ColumnFamily::CoinsByOwnerAndType, &key, &ids)
                .map_err(|e| {
                    error!(
                        owner = %owner,
                        coin_type = %coin_type,
    ...
    #[allow(dead_code)]
    fn remove_from_owner_type_index(
        &self,
        owner: &Address,
        coin_type: &CoinType,
        object_id: &ObjectId,
        ...
        if ids.len() < original_len {
            if ids.is_empty() {
                self.db
                    .delete_raw(ColumnFamily::CoinsByOwnerAndType, &key)
                    .map_err(|e| {
                        error!(
                            owner = %owner,
                            coin_type = %coin_type,
    ...
    fn get_owner_type_index(
        &self,
        owner: &Address,
        coin_type: &CoinType,
    ) -> SetuResult<Vec<ObjectId>> {
        let key = Self::make_owner_cointype_key(owner, coin_type);
        self.db
            .get_raw(ColumnFamily::CoinsByOwnerAndType, &key)
    ...
    fn batch_add_to_owner_type_index(
        &self,
        batch: &mut WriteBatch,
        owner: &Address,
        coin_type: &CoinType,
        ...
        if !ids.contains(object_id) {
            ids.push(*object_id);
            self.db
                .batch_put_raw(batch, ColumnFamily::CoinsByOwnerAndType, &key, &ids)
                .map_err(|e| SetuError::StorageError(e.to_string()))?;
            debug!(
                owner = %owner,
                coin_type = %coin_type,
    ...
    fn batch_remove_from_owner_type_index(
        &self,
        batch: &mut WriteBatch,
        owner: &Address,
        coin_type: &CoinType,
        ...
        if ids.len() < original_len {
            if ids.is_empty() {
                self.db
                    .batch_delete_raw(batch, ColumnFamily::CoinsByOwnerAndType, &key)
                    .map_err(|e| SetuError::StorageError(e.to_string()))?;
            } else {
                self.db
                    .batch_put_raw(batch, ColumnFamily::CoinsByOwnerAndType, &key, &ids)
    ...
    pub fn rebuild_coin_type_index(&self) -> SetuResult<RebuildIndexResult> {
        ...
        let iter = self
            .db
            .iter_values::<Coin>(ColumnFamily::Coins)
            .map_err(|e| {
            ...
            match coin_result {
                Ok(coin) => {
                    if let Some(owner) = &coin.metadata.owner {
                        match self.add_to_owner_type_index(
                            owner,
                            &coin.data.coin_type,
                            &coin.metadata.id,
                        ) {
                            ...
                            Err(e) => {
                                result.failed += 1;
                                let err_msg =
                                    format!("Failed to index coin {}: {}", coin.metadata.id, e);
                                warn!(
    ...
    pub fn clear_and_rebuild_coin_type_index(&self) -> SetuResult<RebuildIndexResult> {
        ...
        info!(
            cleared = cleared,
            "Cleared existing index entries, now rebuilding"
        );
...
impl ObjectStore for RocksObjectStore {
    ...
    fn store_coin(&self, coin: &Coin) -> SetuResult<ObjectId> {
        ...
        self.db
            .batch_put(&mut batch, ColumnFamily::Coins, &id, coin)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
    ...
    fn update_coin(&self, coin: &Coin) -> SetuResult<()> {
        ...
        if let Some(old_coin) = self.get_coin(&coin.metadata.id)? {
            ...
            if old_coin.metadata.owner != coin.metadata.owner {
                if let Some(old_owner) = &old_coin.metadata.owner {
                    self.batch_remove_from_index(
                        &mut batch,
                        ColumnFamily::CoinsByOwner,
                        old_owner,
                        &coin.metadata.id,
                    )?;
                    self.batch_remove_from_owner_type_index(
                        &mut batch,
                        old_owner,
                        &old_coin.data.coin_type,
                        &coin.metadata.id,
                    )?;
                }
                if let Some(new_owner) = &coin.metadata.owner {
                    self.batch_add_to_index(
                        &mut batch,
                        ColumnFamily::CoinsByOwner,
                        new_owner,
                        &coin.metadata.id,
                    )?;
                    self.batch_add_to_owner_type_index(
                        &mut batch,
                        new_owner,
                        &coin.data.coin_type,
                        &coin.metadata.id,
        ...
        self.db
            .batch_put(&mut batch, ColumnFamily::Coins, &coin.metadata.id, coin)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
    ...
    fn delete_coin(&self, id: &ObjectId) -> SetuResult<()> {
        ...
        self.db
            .batch_delete(&mut batch, ColumnFamily::Coins, id)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
    ...
    fn store_profile(&self, profile: &Profile) -> SetuResult<ObjectId> {
        ...
        self.db
            .put(ColumnFamily::Profiles, &id, profile)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
        self.db
            .put(ColumnFamily::ProfileByAddress, &profile.data.owner, &id)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
        Ok(id)
    }
    ...
    fn get_profile(&self, id: &ObjectId) -> SetuResult<Option<Profile>> {
        self.db
            .get(ColumnFamily::Profiles, id)
            .map_err(|e| SetuError::StorageError(e.to_string()))
    }

    fn get_profile_by_address(&self, address: &Address) -> SetuResult<Option<Profile>> {
        let profile_id: Option<ObjectId> = self
            .db
            .get(ColumnFamily::ProfileByAddress, address)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
        match profile_id {
    ...
    fn update_profile(&self, profile: &Profile) -> SetuResult<()> {
        if let Some(old_profile) = self.get_profile(&profile.metadata.id)? {
            if old_profile.data.owner != profile.data.owner {
                self.db
                    .delete(ColumnFamily::ProfileByAddress, &old_profile.data.owner)
                    .map_err(|e| SetuError::StorageError(e.to_string()))?;
            }
        }
    ...
    fn delete_profile(&self, id: &ObjectId) -> SetuResult<()> {
        if let Some(profile) = self.get_profile(id)? {
            self.db
                .delete(ColumnFamily::ProfileByAddress, &profile.data.owner)
                .map_err(|e| SetuError::StorageError(e.to_string()))?;
        }
        self.db
            .delete(ColumnFamily::Profiles, id)
            .map_err(|e| SetuError::StorageError(e.to_string()))
    }
    ...
    fn store_credential(&self, credential: &Credential) -> SetuResult<ObjectId> {
        ...
        self.db
            .put(ColumnFamily::Credentials, &id, credential)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
        self.add_to_index(
            ColumnFamily::CredentialsByHolder,
            &credential.data.holder,
            &id,
        )?;
        self.add_to_index(
            ColumnFamily::CredentialsByIssuer,
    ...
    fn get_credential(&self, id: &ObjectId) -> SetuResult<Option<Credential>> {
        self.db
            .get(ColumnFamily::Credentials, id)
            .map_err(|e| SetuError::StorageError(e.to_string()))
    }
    ...
    fn get_credentials_by_holder(&self, holder: &Address) -> SetuResult<Vec<Credential>> {
        ...
        let creds: Vec<Option<Credential>> = self
            .db
            .multi_get(ColumnFamily::Credentials, &ids)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
    ...
    fn delete_credential(&self, id: &ObjectId) -> SetuResult<()> {
        ...
        self.db
            .delete(ColumnFamily::Credentials, id)
            .map_err(|e| SetuError::StorageError(e.to_string()))
    }
    ...
    fn store_graph(&self, graph: &RelationGraph) -> SetuResult<ObjectId> {
        ...
        self.db
            .put(ColumnFamily::RelationGraphs, &id, graph)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
    ...
    fn get_graph(&self, id: &ObjectId) -> SetuResult<Option<RelationGraph>> {
        self.db
            .get(ColumnFamily::RelationGraphs, id)
            .map_err(|e| SetuError::StorageError(e.to_string()))
    }
    ...
    fn get_graphs_by_owner(&self, owner: &Address) -> SetuResult<Vec<RelationGraph>> {
        ...
        let graphs: Vec<Option<RelationGraph>> = self
            .db
            .multi_get(ColumnFamily::RelationGraphs, &ids)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
    ...
    fn update_graph(&self, graph: &RelationGraph) -> SetuResult<()> {
        ...
        self.db
            .put(ColumnFamily::RelationGraphs, &graph.metadata.id, graph)
            .map_err(|e| SetuError::StorageError(e.to_string()))
    }
    ...
    fn delete_graph(&self, id: &ObjectId) -> SetuResult<()> {
        ...
        self.db
            .delete(ColumnFamily::RelationGraphs, id)
            .map_err(|e| SetuError::StorageError(e.to_string()))
    }
    ...
    fn store_user_relation_network(
        ...
        self.db
            .put(ColumnFamily::UserRelationNetworks, &id, network)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
        // Index by user address
        self.db
            .put(ColumnFamily::UserRelationNetworkByUser, user, &id)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
        Ok(id)
    }
    ...
    fn get_user_relation_network(
        ...
        let network_id: Option<ObjectId> = self
            .db
            .get(ColumnFamily::UserRelationNetworkByUser, user)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
        match network_id {
            Some(id) => self
                .db
                .get(ColumnFamily::UserRelationNetworks, &id)
                .map_err(|e| SetuError::StorageError(e.to_string())),
            None => Ok(None),
        }
    }
    ...
    fn update_user_relation_network(&self, network: &UserRelationNetworkObject) -> SetuResult<()> {
        self.db
            .put(
                ColumnFamily::UserRelationNetworks,
                &network.metadata.id,
                network,
            )
            .map_err(|e| SetuError::StorageError(e.to_string()))
    ...
    fn delete_user_relation_network(&self, user: &Address) -> SetuResult<()> {
        if let Some(network) = self.get_user_relation_network(user)? {
            self.db
                .delete(ColumnFamily::UserRelationNetworks, &network.metadata.id)
                .map_err(|e| SetuError::StorageError(e.to_string()))?;
            self.db
                .delete(ColumnFamily::UserRelationNetworkByUser, user)
                .map_err(|e| SetuError::StorageError(e.to_string()))?;
        }
        Ok(())
    }
    ...
    fn store_user_subnet_activity(&self, activity: &UserSubnetActivity) -> SetuResult<()> {
        ...
        self.db
            .put(ColumnFamily::UserSubnetActivities, &key, activity)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
    ...
    fn get_user_subnet_activity(
        ...
        self.db
            .get(ColumnFamily::UserSubnetActivities, &key)
            .map_err(|e| SetuError::StorageError(e.to_string()))
    }
    ...
    fn get_user_all_subnet_activities(
        ...
        let subnet_ids: Vec<SubnetId> = self
            .db
            .get(ColumnFamily::UserSubnetActivitiesByUser, user)
            .map_err(|e| SetuError::StorageError(e.to_string()))?
            .unwrap_or_default();
    ...
    fn delete_user_subnet_activity(&self, user: &Address, subnet_id: &SubnetId) -> SetuResult<()> {
        let key = Self::make_subnet_activity_key(user, subnet_id);
        self.db
            .delete(ColumnFamily::UserSubnetActivities, &key)
            .map_err(|e| SetuError::StorageError(e.to_string()))?;
...
mod tests {
    use super::*;
    use setu_types::{
        create_kyc_credential, create_profile, create_social_graph, generate_object_id,
    };
    ...
    fn test_coins_by_owner_and_type() {
        ...
        let setu_coins = store
            .get_coins_by_owner_and_type(&alice, &CoinType::native())
            .unwrap();
        assert_eq!(setu_coins.len(), 1);
        assert_eq!(setu_coins[0].value(), 1000);

        let usdc_coins = store
            .get_coins_by_owner_and_type(&alice, &CoinType::new("USDC"))
            .unwrap();
        assert_eq!(usdc_coins.len(), 2);
        let usdc_total: u64 = usdc_coins.iter().map(|c| c.value()).sum();
        assert_eq!(usdc_total, 800);

        let flux_coins = store
            .get_coins_by_owner_and_type(&alice, &CoinType::new("FLUX"))
            .unwrap();
        assert_eq!(flux_coins.len(), 1);

        // Non-existent type returns empty
        let btc_coins = store
            .get_coins_by_owner_and_type(&alice, &CoinType::new("BTC"))
            .unwrap();
    ...
    fn test_coin_type_index_update_on_owner_change() {
        ...
        let bob_usdc = store
            .get_coins_by_owner_and_type(&bob, &CoinType::new("USDC"))
            .unwrap();

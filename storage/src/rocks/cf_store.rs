...
impl RocksDBCFStore {
    ...
    pub fn from_shared(db: Arc<SetuDB>) -> Self {
        ...
        let pending_seq = db
            .get_raw::<u64>(ColumnFamily::ConsensusFrames, key_prefix::META_PENDING_SEQ)
            .ok()
            .flatten()
            .unwrap_or(0);

        let finalized_seq = db
            .get_raw::<u64>(
                ColumnFamily::ConsensusFrames,
                key_prefix::META_FINALIZED_SEQ,
            )
            .ok()
            .flatten()
            .unwrap_or(0);
    ...
    fn pending_key(seq: u64, cf_id: &CFId) -> Vec<u8> {
        ...
        let mut key =
            Vec::with_capacity(key_prefix::PENDING.len() + seq_str.len() + 1 + cf_id.len());
    ...
    fn finalized_key(seq: u64, cf_id: &CFId) -> Vec<u8> {
        ...
        let seq_str = format!("{:016x}", seq);
        let mut key =
            Vec::with_capacity(key_prefix::FINALIZED.len() + seq_str.len() + 1 + cf_id.len());
    ...
    pub async fn store(&self, cf: ConsensusFrame) -> SetuResult<()> {
        ...
        if is_finalized {
            ...
            self.db
                .batch_put_raw(&mut batch, ColumnFamily::ConsensusFrames, &pending_key, &())
                .map_err(|e| SetuError::StorageError(e.to_string()))?;
    ...
    pub async fn get(&self, cf_id: &CFId) -> Option<ConsensusFrame> {
        let cf_key = Self::cf_key(cf_id);
        self.db
            .get_raw(ColumnFamily::ConsensusFrames, &cf_key)
            .ok()
            .flatten()
    }
    ...
    pub async fn mark_finalized(&self, cf_id: &CFId) {
        ...
        let pending_keys = match self
            .db
            .prefix_scan_keys(ColumnFamily::ConsensusFrames, key_prefix::PENDING)
        {
            Ok(keys) => keys,
            Err(_) => return,
        };
        ...
        for key in pending_keys {
            if let Some(id) = Self::extract_cf_id_from_index_key(&key, key_prefix::PENDING) {
                if &id == cf_id {
                    let _ =
                        self.db
                            .batch_delete_raw(&mut batch, ColumnFamily::ConsensusFrames, &key);
                    break;
                }
            }
        }
    ...
    pub async fn latest_finalized(&self) -> Option<ConsensusFrame> {
        ...
        finalized_keys.last().and_then(|key| {
            Self::extract_cf_id_from_index_key(key, key_prefix::FINALIZED).and_then(|cf_id| {

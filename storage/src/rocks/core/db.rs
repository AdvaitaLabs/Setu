...
use super::{ColumnFamily, RocksDBConfig, StorageError};
...
impl SetuDB {
    ...
    pub fn multi_get<K, V>(&self, cf: ColumnFamily, keys: &[K]) -> Result<Vec<Option<V>>>
    where
        K: Encode,
        V: DeserializeOwned,
    {
        let cf_handle = self.cf_handle(cf)?;
        let key_bytes: Vec<_> = keys.iter().map(Self::encode_key).collect::<Result<_>>()?;

        let results = self
            .db
            .multi_get_cf(key_bytes.iter().map(|k| (cf_handle, k.as_slice())));
    ...
    pub fn batch_delete<K>(&self, batch: &mut WriteBatch, cf: ColumnFamily, key: &K) -> Result<()>
    where
        K: Encode,
    {
        let cf_handle = self.cf_handle(cf)?;
    ...
    ///
    /// Without a prefix_extractor on the column family, RocksDB's prefix_iterator_cf
    /// seeks to the prefix position but does NOT stop at the prefix boundary.
    /// We add an explicit take_while filter to enforce correct prefix semantics.
...
mod tests {
    ...
    fn test_batch_write() {
        ...
        for i in 0..10 {
            ...
            db.batch_put(&mut batch, ColumnFamily::Objects, &key, &value)
                .unwrap();
        }

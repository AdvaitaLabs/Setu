...
use setu_merkle::{
    HashValue, MerkleStore, SparseMerkleProof, SparseMerkleTree, SubnetAggregationTree,
    SubnetStateEntry,
};
use setu_types::event::{Event, ExecutionResult, StateChange};
use setu_types::{AnchorMerkleRoots, SubnetId};
...
impl GlobalStateManager {
    ...
    pub fn compute_global_root(&self) -> (HashValue, HashMap<SubnetId, HashValue>) {
        let entries: Vec<SubnetStateEntry> = self
            .subnet_states
            .iter()
        ...
        let subnet_roots: HashMap<SubnetId, HashValue> = self
            .subnet_states
            .iter()
    ...
    pub fn commit(&mut self, anchor_id: u64) -> setu_merkle::MerkleResult<()> {
        ...
        if let Some(ref store) = self.store {
            for (subnet_id, smt) in &self.subnet_states {
                store.put_subnet_root(subnet_id.as_bytes(), anchor_id, &smt.root())?;
            }
    ...
    pub fn apply_state_change(&mut self, subnet_id: SubnetId, change: &StateChange) -> ApplyResult {
    ...
    pub fn apply_committed_events(&mut self, events: &[Event]) -> StateApplySummary {
        ...
        for event in &sorted_events {
            ...
            if let Some(result) = &event.execution_result {
                ...
                // Track in summary
                summary.record_event(subnet_id, &event.id, changes_count, new_root);
            }
        }

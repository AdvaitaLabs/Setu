...
use setu_merkle::{
    BinaryMerkleTree, HashValue as MerkleHash, SubnetAggregationTree, SubnetStateEntry,
};
use setu_types::{Anchor, AnchorMerkleRoots, Event, HashValue as TypesHash, SubnetId};
...
pub fn compute_events_root(events: &[Event]) -> MerkleHash {
    ...
    let leaves: Vec<&[u8]> = sorted_events.iter().map(|e| e.id.as_bytes()).collect();
...
pub fn compute_anchor_chain_root(anchors: &[&Anchor]) -> MerkleHash {
    ...
    let leaves: Vec<&[u8]> = anchors.iter().map(|a| a.id.as_bytes()).collect();
...
pub fn compute_anchor_chain_root_from_hashes(anchor_hashes: &[[u8; 32]]) -> [u8; 32] {
    ...
    let leaves: Vec<&[u8]> = anchor_hashes.iter().map(|h| h.as_slice()).collect();
...
impl AnchorMerkleRootsBuilder {
    ...
    pub fn build(self) -> AnchorMerkleRoots {
        ...
        let subnet_roots_converted: HashMap<SubnetId, TypesHash> = self
            .subnet_roots
            .into_iter()

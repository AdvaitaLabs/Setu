use crate::anchor_builder::{
    AnchorBuildError, AnchorBuildResult, AnchorBuilder, PendingAnchorBuild,
};
use crate::dag::Dag;
use crate::vlc::VLC;
use setu_storage::subnet_state::GlobalStateManager;
use setu_types::{Anchor, ConsensusConfig, ConsensusFrame, EventId, Vote};
...
impl ConsensusManager {
    ...
    pub fn try_create_cf(&mut self, dag: &Dag, vlc: &VLC) -> Option<ConsensusFrame> {
    ...
    pub fn has_cf(&self, cf_id: &str) -> bool {
        self.pending_cfs.contains_key(cf_id) || self.finalized_cfs.iter().any(|cf| cf.id == cf_id)
    }
                    ...
                    if let Some(pending_build) = self.pending_builds.remove(cf_id) {
                        ...
                        match self.anchor_builder.commit_build(pending_build.clone()) {
                            ...
                            Err(AnchorBuildError::SnapshotMismatch { .. }) => {
                                ...
                                if let Err(e) = self
                                    .anchor_builder
                                    .apply_follower_finalized_cf(&events, &cf)
                                {
                                    eprintln!(
                                        "Follower fallback failed: {}, syncing metadata only",
                                        e
                                    );
    ...
    pub fn cleanup_timeout_cfs(&mut self) -> usize {
        ...
        let timeout_ids: Vec<String> = self
            .pending_cfs
            .iter()
    ...
    pub fn apply_cf_state_changes(&mut self, dag: &Dag, cf: &setu_types::ConsensusFrame) -> bool {
        ...
        let events: Vec<setu_types::Event> = cf
            .anchor
            .event_ids
            .iter()
    ...
    pub fn verify_cf_merkle_roots(&self, cf: &setu_types::ConsensusFrame) -> bool {
        ...
        if !merkle_roots.subnet_roots.is_empty() {
            if !merkle_roots
                .subnet_roots
                .contains_key(&setu_types::SubnetId::ROOT)
            {
                return false;
            }
        }

...
use setu_types::{
    object_type, Event, EventType, HashValue as TypesHash, ObjectStateValue, SubnetId, ZERO_HASH,
};
...
impl RootSubnetExecutor {
    ...
    pub fn execute_batch(
        &mut self,
        events: &[Event],
    ) -> Vec<Result<RootExecutionResult, RootExecutorError>> {
    ...
    fn execute_validator_register(
        &mut self,
        event: &Event,
    ) -> Result<RootExecutionResult, RootExecutorError> {
    ...
    fn execute_validator_unregister(
        &mut self,
        event: &Event,
    ) -> Result<RootExecutionResult, RootExecutorError> {
        let key = self.generate_validator_key(&event.creator);
